...
def rel_error(x, y):
    ...
    return np.max(np.abs(x - y) / (np.maximum(1e-8, np.abs(x) + np.abs(y)))) #两个比较，取最大值
...
def init_toy_data():
    np.random.seed(1)
    X = 10 * np.random.randn(num_inputs, input_size)   #5*4
...
print('Final training loss: ', stats['loss_history'][-1])  #last loss

# plot the loss history
plt.plot(stats['loss_history'])   #loss_history: type is list
...
def show_net_weights(net):
    ...
    print(W1.shape)
    W1 = W1.reshape(32, 32, 3, -1).transpose(3, 0, 1, 2)  #?transpose
    print(W1.shape)
