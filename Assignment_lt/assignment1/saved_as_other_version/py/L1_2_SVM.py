...
print('Training data shape: ', X_train.shape)  #数据类型为 ndarray
...
for y, cls in enumerate(classes):
    ...
    idxs = np.random.choice(idxs, samples_per_class, replace=False)  #replace=False  :无重复数出现
...
mean_image = np.mean(X_train, axis=0)   #每一个像素点的均值
...
# second: subtract the mean image from train and test data（用到broadcast）
X_train -= mean_image
X_val -= mean_image
X_test -= mean_image
X_dev -= mean_image
...
X_train = np.hstack([X_train, np.ones((X_train.shape[0], 1))])  #二维数组后面加一列1
...
# learning rate). if you are careful you should be able to
...
for i in range(10):
    ...
    wimg = 255.0 * (w[:, :, :, i].squeeze() - w_min) / (w_max - w_min) #squeeze：Remove single-dimensional entries from the shape of an array
